 ...
 * <a href="http://www.mongodb.org/display/DOCS/Replica+Sets">replica set</a> using the Java driver by passing a ServerAddress list to the MongoClient
 ...
 * </pre> You can connect to a sharded cluster using the same constructor. MongoClient will auto-detect whether the servers are a list of replica set
 ...
 * </pre> By default, all write operations will wait for acknowledgment by the server, as the default write concern is
 * {@code WriteConcern.ACKNOWLEDGED}.
 * <p>
 * Note: This class supersedes the {@code Mongo} class. While it extends {@code Mongo}, it differs from it in that the default write concern is to
 * wait for acknowledgment from the server of all write operations. In addition, its constructors accept instances of {@code MongoClientOptions} and
 * {@code MongoClientURI}, which both also set the same default write concern.
 * <p>
 * In general, users of this class will pick up all of the default options specified in {@code MongoClientOptions}. In particular, note that the
 * default value of the connectionsPerHost option has been increased to 100 from the old default value of 10 used by the superseded {@code Mongo}
 * class.
 *
...
public class MongoClient extends Mongo {
    ...
    /**
     * Creates a Mongo based on a list of replica set members or a list of mongos. It will find all members (the master will be used by default). If
     * you pass in a single server in the list, the driver will still function as if it is a replica set. If you have a standalone server, use the
     * Mongo(ServerAddress) constructor.
     * <p/>
     * If this is a list of mongos servers, it will pick the closest (lowest ping time) one to send all requests to, and automatically fail over to
     * the next server if the closest is down.
     *
     * @param seeds Put as many servers as you can in the list and the system will figure out the rest. This can either be a list of mongod servers in
     * the same replica set or a list of mongos servers in the same sharded cluster.
     * @throws MongoException
     * @see com.mongodb.ServerAddress
     */
