...
//            final String ji过程_s = "i_ID(";
        ...
        @Override
        public void g废弃新记录(final String ci數據表_s) throws java.sql.SQLException {
            java.sql.PreparedStatement jd写操作 = null;
            try {
                if (ci數據表_s != null) {//删除新增记录
                    jd写操作 = d连接.prepareStatement("UPDATE " + ci數據表_s + " SET xt.bj = 1 WHERE (xt).bj = 2 And (id).server = ? And (id).vd = ?", java.sql.ResultSet.TYPE_SCROLL_INSENSITIVE, java.sql.ResultSet.CONCUR_READ_ONLY, java.sql.ResultSet.HOLD_CURSORS_OVER_COMMIT);
